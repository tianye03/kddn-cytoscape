...
public class BCD {
	...
	if (rho2 <= (rho1 + 2*lambda2) && rho2 >= (rho2 - 2*lambda2)
	    && rho2 >= (2*lambda1[k] - rho1)) {
	    b1 = (rho1 + rho2)/2 - lambda1[k];
	    b2 = (rho1 + rho2)/2 - lambda1[k];
	}

	if (rho2 > (rho1 + 2*lambda2) && rho1 >= (lambda1[k] - lambda2)) {
	    b1 = rho1 - lambda1[k] + lambda2;
	    b2 = rho2 - lambda1[k] - lambda2;
	}

	if (rho1 < (lambda1[k] - lambda2) && rho1 >= -(lambda1[k] + lambda2)
	    && rho2 >= (lambda1[k] + lambda2)) {
	    b1 = 0;
	    b2 = rho2 - lambda1[k] - lambda2;
	}

	if (rho1 < -(lambda1[k] + lambda2) && rho2 >= (lambda1[k] + lambda2)) {
	    b1 = rho1 + lambda1[k] + lambda2;
	    b2 = rho2 - lambda1[k] - lambda2;
	}
	...
	if (rho2 < -(lambda1[k] - lambda2) && rho2 >= (rho1 + 2*lambda2)) {
	    b1 = rho1 + lambda1[k] + lambda2;
	    b2 = rho2 + lambda1[k] - lambda2;
	}
	...
	if (rho2 > -(lambda1[k] + lambda2) && rho2 <= (lambda1[k] - lambda2)
